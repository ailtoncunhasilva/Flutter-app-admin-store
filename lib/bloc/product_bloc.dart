...
class ProductBloc extends BlocBase{
  ...
  void savePrice(String price){
    unsavedData['price'] = double.tryParse(price);
  }
  ...
  Future<bool> saveProduct() async{
    ...
    try{
      if(product != null){
        ...
        DocumentReference dr = await Firestore.instance.collection('store02').
  ...
  Future _uploadImages(String productId) async{
    for(int i = 0; i < unsavedData['images'].length; i++){
      ...
      StorageUploadTask uploadTask = FirebaseStorage.instance.ref().child(productId).
        child(DateTime.now().millisecondsSinceEpoch.toString()).putFile(unsavedData['images'][i]);
